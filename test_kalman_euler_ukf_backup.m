...
figure(1);
subplot(2,2,1);
view1 = OrientationView('kalman filter', gca);
subplot(2,2,2);
view2 = OrientationView('EKF', gca);
subplot(2,2,3);
view3 = OrientationView('UKF', gca);
subplot(2,2,4);
view4 = OrientationView('Google filter', gca);
...
for k = 1:nSamples
    ...
    quatEKF = euler2quaternion(phi, theta, psi, 'xyz');
...
ukfPhiSaved = ukfEulerSaved(:,1)*180/pi;
ukfThetaSaved = ukfEulerSaved(:,2)*180/pi;
ukfPsiSaved = ukfEulerSaved(:,3)*180/pi;
...
plot(tkalman, gyroPhiSaved, tkalman, accelPhiSaved, tkalman, kalmanPhiSaved, tkalman, ...
    ekfPhiSaved, tkalman, ukfPhiSaved);
legend('Gyroscope', 'Accleration', 'Kalman', 'EKF', 'UKF'); ylabel('Phi');

figure
plot(tkalman, gyroThetaSaved, tkalman, accelThetaSaved, tkalman, kalmanThetaSaved, ...
    tkalman, ekfThetaSaved, tkalman, ukfThetaSaved);
legend('Gyroscope', 'Accleration', 'Kalman', 'EKF', 'UKF'); ylabel('Theta');

figure
plot(tkalman, gyroPsiSaved, tkalman, kalmanPsiSaved, tkalman, ...
    ekfPsiSaved, tkalman, ukfPsiSaved);
legend('Gyroscope', 'Accleration', 'Kalman', 'EKF', 'UKF'); ylabel('Psi');
